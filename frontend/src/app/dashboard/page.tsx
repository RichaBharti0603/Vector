      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-10 border-b border-slate-800 pb-6">
          <div>
            ...
            <p className="text-slate-400 mt-1">Autonomous Workplace Operations</p>
          </div>
          <button 
            onClick={triggerDemo}
                ...
                events.map((event, i) => (
                  <div key={i} className="flex flex-col p-3 rounded-lg bg-slate-950 border border-slate-800">
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-xs font-bold text-indigo-400 uppercase">{event.payload.agent_type}</span>
                    ...
                    <span className="text-sm text-slate-200">{event.payload.message}</span>
                  </div>
                ))
              )}
            </div>
